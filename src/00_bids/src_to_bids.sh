...
chmod -R u+rw "${BIDS_DIR}/sub-${SUBJECT}/fmap"

func_files_json=$(cat <<EOF
[
...
for j_path in "${BIDS_DIR}/sub-${SUBJECT}"/fmap/sub-"${SUBJECT}"_*.json; do
  [ -e "$j_path" ] || continue
  tmp=$(mktemp)
  jq --argjson arr "${func_files_json}" '.IntendedFor = $arr' "$j_path" > "$tmp"
  mv "$tmp" "$j_path"
  echo "Patched IntendedFor in $(basename "$j_path")"
done

echo "Cleaning func JSONs (AcquisitionDuration + TaskName)..." #AcquisitionDuration causes BIDS Validation issues (RepetitionTime is enough), TaskName missing
chmod u+rw "${BIDS_DIR}/sub-${SUBJECT}"/func/sub-"${SUBJECT}"_task-*_bold.json

for j in "${BIDS_DIR}/sub-${SUBJECT}"/func/sub-"${SUBJECT}"_task-*_bold.json; do
  [ -e "$j" ] || continue
  fname=$(basename "$j")

  if [[ "$fname" == *"_task-ES_"* ]]; then
    taskname="ES"
  elif [[ "$fname" == *"_task-rest_"* ]]; then
    taskname="rest"
  else
    taskname=""
  fi
  ...
  tmp=$(mktemp)
  if [[ -n "$taskname" ]]; then
    jq --arg tn "$taskname" 'del(.AcquisitionDuration) | .TaskName = $tn' "$j" > "$tmp"
  else
    jq 'del(.AcquisitionDuration)' "$j" > "$tmp"
  fi
  mv "$tmp" "$j"
  echo "Updated $(basename "$j")"
done

# Files not needed for BIDS, and they can generate issues
if [ -e "${BIDS_DIR}/task-ES_bold.json" ]; then
  rm "${BIDS_DIR}/task-ES_bold.json"
fi
...
echo "Modify coordsystem.json to be BIDS compliant"
  if [ -f "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-rest_coordsystem.json" ]; then
    mv "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-rest_coordsystem.json" \
       "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_coordsystem.json"
  fi
  if [ -f "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-rest_electrodes.tsv" ]; then
    mv "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-rest_electrodes.tsv" \
       "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_electrodes.tsv"
  fi
  rm -f "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-ES_coordsystem.json" \
      "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_task-ES_electrodes.tsv"

for f in "${BIDS_DIR}/sub-${SUBJECT}/eeg/sub-${SUBJECT}_coordsystem.json"; do
  jq '
    .EEGCoordinateSystem = "Other"
    | .EEGCoordinateSystemDescription =
        "Electrode positions generated by the EEG conversion tool; internally labeled as BESA."
  ' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
done
